...
selection.a.Cut('pt_cut','Dijet_pt[0] > 350 && Dijet_pt[1] > 350')
selection.a.Cut('topTag','Dijet_particleNet_TvsQCD[0] > 0.05 && Dijet_particleNet_TvsQCD[1] > 0.05')
...
NCuts.Add('topTag_cut','Dijet_particleNet_TvsQCD[0] > 0.2 && Dijet_particleNet_TvsQCD[1] > 0.2')
# dummy cuts so we can plot values of interest
NCuts.Add('Apt_cut','Diphoton_pt[0] > 25 && Diphoton_pt[1] > 25')
NCuts.Add('deltaR_cut','deltaR < 2.4 && deltaR > 0.8')
...
NCuts.Add('photonMvaID_cut','Diphoton_mvaID[0] > -1.01 && Diphoton_mvaID[1] > -1.01')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_v3_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
    'mtop':[25,0,300],
#    'mtphi':[25,800,2200],
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('photonMvaID' not in nkey) and ('tmass' not in nkey) and ('Apt' not in nkey) and ('Smass' not in nkey) and ('deltaR' not in nkey) and ('photonTag' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
