...
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
    ...
    out = ROOT.TFile.Open('rootfilesMCsublead/THselection_mvaID_HT%s_%s%s_%s%sCorr.root'%(args.HT, args.setname,
	...
	# rkey: SR/CR, pfkey: pass/loose/fail
            ...
            for rkey,rpair in {"SR":passfailSR_SF,"SR1":passfailCR_SF}.items():
              for pfkey,n in rpair.items():
                ...
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_mvaID'),26,425,3025,26,425,3025),['mth2','mth1'])
                templates.Do('Write')
