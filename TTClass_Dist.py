...
#        self.a.Define('useyear','"2016"')
#        self.a.Define('PhotonEffSF','getPhotonSF(Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,0,useyear)')
#        self.a.Define('PhotonEffSF1','getPhotonSF(Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,1,20%s)'%self.year)
#        self.a.Define('PhotonEffSF2','getPhotonSF(Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,2,20%s)'%self.year)
...
#            passFailSF["failfail"] = self.a.Cut('STagCR_failfail_SF','(Diphoton_{2}[0] < {3} && Diphoton_{2}[1] < {3}) && ((Dijet_{0}[0] > {1} && Dijet_{0}[1] <{1}) || (Dijet_{0}[1] > {1} && Dijet_{0}[0] <{1}))'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
    ...
    def ApplySTagTopTagTag(self, SRorCR, Toptagger, ToptaggerWP, Stagger, StaggerWP):
        ...
        if (SRorCR == 'SR'):
            ...
            print('SR: effSF = {}'.format(effSF*100))
            passFail["pass"] = self.a.Cut('STagSR_pass','(DiphotonTag_{2}[0] >= {3}) && (DiphotonTag_{2}[1] >= {3}) && (Dijet_{0}[0] > {1}) && (Dijet_{0}[1] > {1})'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
            self.higgsP_SR = self.getNweighted()
            self.AddCutflowColumn(self.higgsP_SR, "higgsP_SR")
...
#            effnorm = effSFpass/effpass
            self.a.SetActiveNode(checkpoint)
            passFail["fail"] = self.a.Cut('STagSR_fail','((DiphotonTag_{2}[0] >= {3} && DiphotonTag_{2}[1] < {3}) || (DiphotonTag_{2}[1] >= {3} && DiphotonTag_{2}[0] < {3})) && (Dijet_{0}[0] > {1} && Dijet_{0}[1] > {1})'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
            self.higgsF_SR = self.getNweighted()
            self.AddCutflowColumn(self.higgsF_SR, "higgsF_SR")
            ...
            print('SR: effSFfail = {}'.format(effSFfail*100))
            self.a.SetActiveNode(checkpoint)
            passFail["failfail"] = self.a.Cut('STagSR_failfail','(DiphotonTag_{2}[0] < {3} && DiphotonTag_{2}[1] < {3}) && (Dijet_{0}[0] > {1} && Dijet_{0}[1] > {1})'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
            ...
            print('CR: eff = {}'.format(eff*100.))
            passFail["pass"] = self.a.Cut('STagCR_pass','(DiphotonTag_{2}[0] >= {3} && DiphotonTag_{2}[1] >= {3}) && ((Dijet_{0}[0] > {1} && Dijet_{0}[1] <{1}) || (Dijet_{0}[1] > {1} && Dijet_{0}[0] <{1}))'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
            self.higgsP_CR = self.getNweighted()
            self.AddCutflowColumn(self.higgsP_CR, "higgsP_CR")
        # Higgs Fail + cutflow info
            self.a.SetActiveNode(checkpoint)
            passFail["fail"] = self.a.Cut('STagCR_fail','((DiphotonTag_{2}[0] >= {3} && DiphotonTag_{2}[1] < {3}) || (DiphotonTag_{2}[1] >= {3} && DiphotonTag_{2}[0] < {3})) && ((Dijet_{0}[0] > {1} && Dijet_{0}[1] <{1}) || (Dijet_{0}[1] > {1} && Dijet_{0}[0] <{1}))'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
            self.higgsF_CR = self.getNweighted()
            self.AddCutflowColumn(self.higgsF_CR, "higgsF_CR")
            self.a.SetActiveNode(checkpoint)
            passFail["failfail"] = self.a.Cut('STagCR_failfail','(DiphotonTag_{2}[0] < {3} && DiphotonTag_{2}[1] < {3}) && ((Dijet_{0}[0] > {1} && Dijet_{0}[1] <{1}) || (Dijet_{0}[1] > {1} && Dijet_{0}[0] <{1}))'.format(Toptagger, ToptaggerWP, Stagger, StaggerWP))
