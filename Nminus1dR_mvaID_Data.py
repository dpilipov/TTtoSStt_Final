...
from TTClassdRData import TTClassdRData
...
selection = TTClassdRData('raw_nano/%s_%s.txt'%(args.setname,args.era),args.era,args.ijob,args.njobs)
...
selection.a.Cut('photonMvaID','Diphoton2_mvaID[0] > 0.0 && Diphoton2_mvaID[1] > 0.0')
...
jets.Add('photonMvaID2','Diphoton2_mvaID[1]')
jets.Add('photonMvaID1','Diphoton2_mvaID[0]')
#jets.Add('photonMvaIDave','(Diphoton2_mvaID[0]+Diphoton2_mvaID[1])/2.0')
...
#selection.a.Cut('Smass_cut','Smass > 75.0')
#selection.a.Cut('mth_cut','max(mth1,mth2) > 475.0')
...
plotting_vars.Add('phMvaID1','Diphoton2_mvaID[0]')
plotting_vars.Add('phMvaID2','Diphoton2_mvaID[1]')
#plotting_vars.Add('phMvaIDave','photonMvaIDave')
...
#NCuts.Add('dRselsig','dRselsig > -1.0')
#NCuts.Add('dRsignal','dRsignal > -1.0')
...
#NCuts.Add('pt0_cut','Dijet_pt[0] > 350 && Dijet_pt[1] > 350')
...
#NCuts.Add('photonMvaID_cut','Diphoton2_mvaID[0] > 0.0 && Diphoton2_mvaID[1] > 0.0')
NCuts.Add('phMvaID1_cut','(Diphoton2_mvaID[0]>0.0)&&(Smass > 25.0)&&(max(mth1,mth2) > 475.0)')
NCuts.Add('phMvaID2_cut','(Diphoton2_mvaID[1]>0.0)&&(Smass > 25.0)&&(max(mth1,mth2) > 475.0)')
#NCuts.Add('phMvaIDave_cut','photonMvaIDave>-1.0')
...
oFile = ROOT.TFile.Open('rootfilesdR/NMinus1_dRwp_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
#    'mth1':[25,200,3025],
#    'mth2':[25,200,3025],
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('pt0' not in nkey) and ('phMvaID1' not in nkey) and ('phMvaID2' not in nkey) and ('phMvaIDave' not in nkey) and ('mth'  not in nkey) and ('dRselection1'  not in nkey) and ('dRgen'  not in nkey) and ('dR0'  not in nkey) and ('dR1'  not in nkey) and ('dRsignal' not in nkey) and ('dRreflection' not in nkey) and ('dtopR' not in nkey) and ('dRselsig' not in nkey) and ('mthsel1' not in nkey) and ('mthsig' not in nkey) and ('mthref' not in nkey) and ('mth1x' not in nkey) and ('mth2x' not in nkey) and ('dRselection2' not in nkey) and ('deltaR' not in nkey) and ('mthsel2' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
