...
from TTClassSIGX import TTClassSIGX
...
selection = TTClassSIGX('raw_nano/%s_%s.txt'%(args.setname,args.era),args.era,args.ijob,args.njobs)
...
jets.Add('dphij0','abs(Dijet_phi[0]-(Diphoton2_phi[0]+Diphoton2_phi[1]))')
jets.Add('dphij1','abs(Dijet_phi[1]-(Diphoton2_phi[0]+Diphoton2_phi[1]))')

# Plotting variables
plotting_vars = VarGroup('plotting_vars')
#DP EDIT
plotting_vars.Add('dphigen','abs(GenPart_phi[topID]-GenPart_phi[StoAAid0])')
plotting_vars.Add('dphi0','dphij0')
plotting_vars.Add('dphi1','dphij1')
...
plotting_vars.Add('dphiselection1','ichoice*dphij0+(1-ichoice)*dphij1')
plotting_vars.Add('dphiselection2','ichoiceX*dphij1+(1-ichoiceX)*dphij1')
jets.Add('ichoicegentop','int(dtopRj0<dtopRj1)')
plotting_vars.Add('dphisignal','ichoicegentop*dphij0+(1-ichoicegentop)*dphij1')
plotting_vars.Add('dphireflection','ichoicegentop*dphij1+(1-ichoicegentop)*dphij0')
plotting_vars.Add('dphiselsig','int(ichoice==ichoicegentop)*(ichoice*dphij0+(1-ichoice)*dphij1)')
...
NCuts.Add('dphigen','dphigen > -1.0')
NCuts.Add('dphi0','dphij0 > -1.0')
NCuts.Add('dphi1','dphij1 > -1.0')
NCuts.Add('dphiselection1','dphiselection1 > -1.0')
NCuts.Add('dphiselection2','dphiselection2 > -1.0')
NCuts.Add('dphiselsig','dphiselsig > -1.0')
NCuts.Add('dphisignal','dphisignal > -1.0')
NCuts.Add('dphireflection','dphireflection > -1.0')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_dphiselCOMB_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
#    'mth1':[25,200,3025],
#    'mth2':[25,200,3025],
...
#    'mth2x':[40,200,3025],
    ...
    'dphi0':[10,0,4.5],
    'dphi1':[10,0,4.5],
    'dphiselection1':[10,0,4.5],
    'dphiselection2':[10,0,4.5],
    'dphiselsig':[10,0,4.5],
    'dphisignal':[10,0,4.5],
    'dphireflection':[10,0,4.5],
    'dtopR':[40,0,4.5],
#    'mtop':[25,0,300],
#    'mtphi':[25,800,2200],
#    'mphi':[25,50,270],
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('mth'  not in nkey) and ('dphiselection1'  not in nkey) and ('dphigen'  not in nkey) and ('dphi0'  not in nkey) and ('dphi1'  not in nkey) and ('dphisignal' not in nkey) and ('dphireflection' not in nkey) and ('dtopR' not in nkey) and ('dphiselsig' not in nkey) and ('mthsel1' not in nkey) and ('mthsig' not in nkey) and ('mthref' not in nkey) and ('mth1x' not in nkey) and ('mth2x' not in nkey) and ('dphiselection2' not in nkey) and ('deltaR' not in nkey) and ('mthsel2' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
