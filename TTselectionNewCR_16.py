...
#            eA1CR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='cr2')
            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2016",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_CR_PASS, eff_CR_FAIL))
...
#            eA1SR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='sr2')
            selection.a.Define('DiPhotonCatSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2016",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_SR_PASS, eff_SR_FAIL))
	...
	# rkey: SR/CR, pfkey: pass/loose/fail
            ...
            for rkey,rpair in {"SR":passfailSR_SF}.items():
              for pfkey,n in rpair.items():
                ...
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_cutBased'),16,25,825,54,325,3025),['Smass','mth'])
                templates.Do('Write')
...
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
    args.trigEff = Correction("TriggerEff16",'EffLoader_2DfittedHist.cc',['out_Eff_2016APV.root','Eff_2016APV'],corrtype='weight')
#    else:
