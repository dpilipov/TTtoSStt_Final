...
#    selection = THClass(fName,year if 'B' not in year else '17',1,1)
    ...
    # Baseline - no tagging
    hists.Add('preTagDenominator',selection.a.DataFrame.Histo2D(('preTagDenominator','',20,600,2200,20,0,800),'mth_trig','Smass_trig'))
    hists.Add('preTagDenominatorZoomed', selection.a.DataFrame.Histo2D(('preTagDenominatorZoomed','',20,600,2200,20,0,800),'mth_trig','Smass_trig'))
    selection.ApplyTrigs()
    hists.Add('preTagNumerator',selection.a.DataFrame.Histo2D(('preTagNumerator','',20,600,2200,20,0,800),'mth_trig','Smass_trig'))
    hists.Add('preTagNumeratorZoomed', selection.a.DataFrame.Histo2D(('preTagNumeratorZoomed','',20,600,2200,20,0,800),'mth_trig','Smass_trig'))
    ...
    for name,eff in effs.items():
        ...
        g.SetTitle(name)
        g.GetXaxis().SetTitle('m_{j \gamma \gamma} (GeV)')
        g.GetYaxis().SetTitle('m_{\gamma \gamma} (GeV)')
        g.GetZaxis().SetTitle('Efficiency')
        g.SetMinimum(0.6)
        g.SetMaximum(1.0)
        f = ROOT.TF2("eff_func","1-[0]/10*exp([1]*x/1000)*exp([2]*y/200)",60,260,800,2600)
