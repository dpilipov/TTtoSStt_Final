...
def generateCutflow(setName, HT=0, selection=True):
    '''
...
#       labels.extend(["FF_CR","F_CR","P_CR","FF_SR","F_SR","P_SR"])
    ...
    for s in sets:
        print(s)
        res = generateCutflow(s, 0, selection)
        print('\n------------------ {} -------------------'.format(s))
...
#DP EDIT ... just for now!!! eventually will have more samples...
	res = generateCutflow('{}{}x{}'.format(prefix, mT, mPhi), 0, selection)
...
#    mPhi = 125
    mPhi = 300
    Tmasses = [1000,1300,1500]
#    Tmasses = [1000]
    for Tmass in Tmasses:
	nProc = 0
#DP EDIT
	res = generateCutflow('{}{}x{}'.format(prefix, Tmass, mPhi), 0, selection)
