...
signals = ['StoAA1000x100']
#for sig in signals:
#    samples.update({sig : 'b*_{LH} %s (GeV)'%(sig[-4:])})
#for sig in signals:
#    samples.update({sig : ''})
samples.update({'StoAA1000x100' : '1000/100'})
...
#        'pt'             : 'p_{T}^{jet0}',
#        'AptCutBased'                      : 'p_{T}^{#gamma 0}',
#        'AptMvaID'                      : 'p_{T}^{#gamma 0}',
#        'sublead_jetPt'          : 'p_{T}^{jet1}',
#        'deltaphi'               : '#Delta#phi_{jet0,jet1}',
#        'deltaEta'               : '#Delta#eta_{jet0,jet1}',
#        'AdeltaEta'                : '#Delta#eta_{#gamma 0,#gamma 1}',
#        'mtop'     : 'm_{SD}^{jet0}',
#        'SmassCutBased'                   : 'M_{#gamma #gamma}',
        'SmassMvaID'                   : 'M_{#gamma #gamma}',
#        'TPmassCutBased'                   : 'M_{t#gamma #gamma}',
        'TPmassMvaID'                   : 'M_{t#gamma #gamma}',
#        'METpt'                   : 'p_{T}^{MET}',
        'METsumEt'                   : 'sumEt^{MET}',
...
#        'photonMvaID'             : 'Photon mvaID^{#gamma 0}',
#        'photonCutBased'          : 'Photon cutBased^{#gamma 0}',
#        'tmass'                   : 'M_{jj}',
...
#        inFile = ROOT.TFile.Open('rootfiles/NMinus1_MET_{}_{}{}.root'.format(sample, args.year))
        inFile = ROOT.TFile.Open('rootfiles/NMinus1_MET_{}_{}_1of1.root'.format(sample, args.year))
	if inFile == None:
#	    print('WARNING: rootfiles/NMinus1_jets_{}_{}{}.root does not exist, please create first\n'.format(sample, args.year, '_Nminus1_jets' if args.nminus1 else ''))
           print('WARNING: rootfiles/NMinus1_MET_{}_{}_1of1.root does not exist, please create first\n'.format(sample, args.year))
	   continue	
	...
	plot_filename = 'plots/{}_{}{}{}{}.png'.format(varname, args.year, '_Nminus1_MET' if args.nminus1 else '','_SoverB' if args.soverb else '', '_logy' if args.logy else '')
...
#        for bkg in ['QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000','ttbar','singletop_tW','singletop_tWB']:
        tot_bkg_int = 0
#	for bkg in ['Data','QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000','ttbar']:
        for bkg in ['ttgJets','ttgg']:
