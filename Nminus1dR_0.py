...
selection = TTClassREF_0('raw_nano/%s_%s.txt'%(args.setname,args.era),args.era,args.ijob,args.njobs)
...
jets.Add('mthS','ichoicegentop*mth1+(1-ichoicegentop)*mth2')
jets.Add('mthR','ichoicegentop*mth2+(1-ichoicegentop)*mth1')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_dRselCOMB_{}_{}_{}of{}_0.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
#    'mth1':[25,200,3025],
#    'mth2':[25,200,3025],
...
print('Plotting')
c = ROOT.TCanvas('c','c')
graphX = ROOT.TGraph(len(mthR), ROOT.array('d',mthR), ROOT.array('d', mthS))
graphX.Draw("AP")
c.Print('mth2d.pdf')
print(nminus1Nodes)
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('mth'  not in nkey) and ('dRselection1'  not in nkey) and ('dRgen'  not in nkey) and ('dR0'  not in nkey) and ('dR1'  not in nkey) and ('dRsignal' not in nkey) and ('dRreflection' not in nkey) and ('dtopR' not in nkey) and ('dRselsig' not in nkey) and ('mthsel1' not in nkey) and ('mthsig' not in nkey) and ('mthref' not in nkey) and ('mth1x' not in nkey) and ('mth2x' not in nkey) and ('dRselection2' not in nkey) and ('deltaR' not in nkey) and ('mthsel2' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
...
oFile.Close()
#c = ROOT.TCanvas('c','c')
#graphX = ROOT.TGraph(len(mthref), ROOT.array('d',mthref), ROOT.array('d', mthsig))
#graphX.Draw("AP")
#c.Print('mth2d.pdf')
