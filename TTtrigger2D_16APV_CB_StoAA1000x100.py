...
#    else:
    fName = 'dijet_nano/StoAA1000x100_16APV_snapshot.txt'
...
#    selection = TTClass(fName,year if 'B' not in year else '17',1,1)
    ...
    # Baseline - no tagging
#    hists.Add('preTagDenominator',selection.a.DataFrame.Histo2D(('preTagDenominator','',8,25,825,24,625,3025),'Smass_trig','mth_trig'))
...
#    hists.Draw("colz")
    ...
    out = ROOT.TFile.Open('THtrigger2D_HT{}_{}_StoAA1000x100.root'.format(HT,year), 'RECREATE')
    out.cd()
...
#DP EDIT -- only left 16 below...
    files = {
        '16APV': ROOT.TFile.Open('THtrigger2D_HT{}_16APV.root'.format(args.HT))
