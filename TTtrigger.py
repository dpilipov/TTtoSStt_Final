...
def MakeEfficiency(year, HT=0):
    ...
    selection = TTClass(fName,year if year.isdigit() else '17',1,1)
...
#    hists = {hname.GetName():[files[y].Get(hname.GetName()) for y in ['16','17','18']] for hname in files['16'].GetListOfKeys() if '_graph' in hname.GetName()}
    hists = {hname.GetName():[files[y].Get(hname.GetName()) for y in ['16']] for hname in files['16'].GetListOfKeys() if '_graph' in hname.GetName()}
...
#DP EDIT
#DP    leg2 = ROOT.TLegend(0.7,0.5,0.88,0.7)
    # total 2017 minus 2017B
#DP    h17 = files['17'].Get('Pretag_graph')
#DP    h17.SetLineColor(ROOT.kGreen)
#DP    h17.SetTitle('')
#DP    h17.GetXaxis().SetTitle('m_{jj}')
#DP    h17.GetYaxis().SetTitle('Efficiency')
#DP    h17.Draw('AP')
#DP    leg2.AddEntry(h17, 'later 2017', 'pe') 

    # only 2017B
#DP    h17B = files['17B'].Get('Pretag_graph')
#DP    h17B.SetLineColor(ROOT.kBlack)
#DP    h17B.SetTitle('')
#DP    h17B.GetXaxis().SetTitle('m_{jj}')
#DP    h17B.GetYaxis().SetTitle('Efficiency')
#DP    h17B.Draw('same P')
#DP    leg2.AddEntry(h17B, 'full 2017', 'pe')

#DP    leg2.Draw()
#DP    c.Print('plots/Trigger_2017Full_vs_2017Later_HT{}.pdf'.format(args.HT))
