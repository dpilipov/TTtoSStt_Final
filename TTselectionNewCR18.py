...
#DP EDIT
from TTClass18 import TTClass18
...
#    selection = THClass('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    selection = TTClass18('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
...
#            eA1CR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='cr2')
            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2018",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_CR_PASS, eff_CR_FAIL))
#            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(DiPhotonCat,DiphotoniTag_pt,DiphotonTag_eta,DiphotonTag_cutBased,1.0,{0},{1})'.format(eff_CR_PASS, eff_CR_FAIL))
...
#            eA1SR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='sr2')
            selection.a.Define('DiPhotonCatSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2018",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_SR_PASS, eff_SR_FAIL))
#            selection.a.Define('DiPhotonCatSF','updatePhotonTag(DiPhotonCat,DiphotonTag_pt,DiphotonTag_eta,DiphotonTag_cutBased,1.0,{0},{1})'.format(eff_SR_PASS, eff_SR_FAIL))
#            passfailSR = selection.ApplySTagTopTag('SR', top_tagger, 0.8, photon_tagger, 1)
	...
	# rkey: SR/CR, pfkey: pass/loose/fail
            ...
            print('ABOUT TO PLOT....')
            for rkey,rpair in {"SR":passfailSR_SF}.items():
              for pfkey,n in rpair.items():
                ...
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_cutBased'),16,25,825,48,625,3025),['Smass','mth'])
                templates.Do('Write')
...
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
    args.trigEff = Correction("TriggerEff18",'EffLoader_2DfittedHist.cc',['out_Eff_2016APV.root','Eff_2016APV'],corrtype='weight')
#    else:
