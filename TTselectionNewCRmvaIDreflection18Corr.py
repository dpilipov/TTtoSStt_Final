...
#DP EDIT
from TTClassREF18_0 import TTClassREF18_0
...
#    selection = THClass('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    selection = TTClassREF18_0('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
...
#    selection.a.Cut('samebin_cut','samebin < 1')
...
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
    ...
    print('SEL: at out')
    out = ROOT.TFile.Open('rootfilesREF/THselection_mvaID_HT%s_%sref%s_%s%sCorr.root'%(args.HT, args.setname,
...
#                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_mvaID'),16,25,825,26,425,3025),['Smass','mth'])
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_mvaID'),26,425,3025,26,425,3025),['mth','mthsig'])
                templates.Do('Write')
