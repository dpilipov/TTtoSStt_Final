...
#      if (year==16):
    ...
    if selection:
        ...
        for year, varDict in years.items():
#          if (year==16):
	    if not os.path.exists('root://cmseos.fnal.gov//store/user/dpilipov/topHBoostedAllHad/selection/THselection_mvaID_dR_HT{}_{}_{}.root'.format(HT, setName, year)):
	        pass
	    f = ROOT.TFile.Open('root://cmseos.fnal.gov//store/user/dpilipov/topHBoostedAllHad/selection/THselection_mvaID_dR_HT{}_{}_{}.root'.format(HT, setName, year))
            print(f)
	    h = f.Get('cutflow')
...
#    sets = ["StoAA800x150","StoAA900x150","StoAA1000x150","StoAA1100x150","StoAA1200x150","StoAA1300x150","StoAA1400x150","StoAA1500x150"]
    sets = ["StoAA900x150"]
    for s in sets:
        print(s)
...
#	    latexRow = '{}{}'.format(prefix,mPhi) + " &"
            print(year)
            print(mPhi)
#            if not((year=='16APV')and(mPhi==350)) and not((year=='17')and(mPhi==50)) and not((year=='18')and(mPhi==150 or mPhi==250)):
            if not(mPhi==350) and not(mPhi==50):  
	      for var, val in dicts.items():
		if (var == 'NPROC'):
		    nProc = val
		    latexRow += " 1 &"
...
#    Tmasses = [800, 900, 1000, 1200, 1300, 1500]
    Tmasses = [800,900,1000,1100,1200,1300,1400,1500]
