...
#from TTClass_Dist import TTClass_Dist
...
#    'HT': 'Sum of lead and sublead jet p_{T}',
    ...
    'dimass0': 'Leading AK8 jet softdrop mass',
    'dimass1': 'Sublead AK8 jet softdrop mass',
...
#    'diTag0': 'top tagger TvsQCD leading jet',
#    'diTag1': 'top tagger TvsQCD subleading jet',
    'diApt0': 'Leading photon p_{T}',
    'diApt1': 'Sublead photon p_{T}',
#    'HT': 'Sum of lead and sublead jet p_{T}',
    'diAeta0': 'Leading photon #eta',
    'diAeta1': 'Sublead photon #eta',
    'diAphi0': 'Leading photon #varphi',
    'diAphi1': 'Sublead photon #varphi',
#    'diATag0': 'photon tagger cutBased leading photon',
#    'diATag1': 'photon tagger cutBased subleading photon',
    'TPmass': 'TP invariant mass M_{#gamma #gamma t}',
    'AAmass': 'Diphoton invariant mass m_{#gamma #gamma}'
}
...
#    selection = TTClass_Dist('dijet_nano/%s_%s_snapshot.txt'%(setname,year),year,1,1)
...
#DP EDIT
    selection.a.Cut('highHT','HT > 900')
    selection.a.Define('dipt0','Dijet_pt_corr[0]')
    selection.a.Define('dipt1','Dijet_pt_corr[1]')
...
#    selection.a.Define('sublead_vect','hardware::TLvector(Dijet_pt[1], Dijet_eta[1], Dijet_phi[1], Dijet_msoftdrop[1])')
    selection.a.Define('lead_vect',"hardware::TLvector(Dijet_pt_corr[0], Dijet_eta[0], Dijet_phi[0], Dijet_msoftdrop_corrT[0])")
    selection.a.Define('sublead_vect','hardware::TLvector(Dijet_pt_corr[1], Dijet_eta[1], Dijet_phi[1], Dijet_msoftdrop_corrT[1])')
#    selection.a.Define('dimasstt','hardware::InvariantMass({lead_vect,sublead_vect})')
#    selection.a.Define('diApt0','Diphoton_pt[0]')
#    selection.a.Define('diApt1','Diphoton_pt[1]')
#    selection.a.Define('diAeta0','Diphoton_eta[0]')
#    selection.a.Define('diAeta1','Diphoton_eta[1]')
#    selection.a.Define('diAphi0','Diphoton_phi[0]')
#    selection.a.Define('diAphi1','Diphoton_phi[1]')
#    selection.a.Cut('highHT','HT > 0')
    selection.a.Define('diApt0','Diphoton_pt[0]')
    selection.a.Define('diApt1','Diphoton_pt[1]')
    selection.a.Define('diAeta0','Diphoton_eta[0]')
    selection.a.Define('diAeta1','Diphoton_eta[1]')
    selection.a.Define('diAphi0','Diphoton_phi[0]')
    selection.a.Define('diAphi1','Diphoton_phi[1]')
    selection.a.Define('leadA_vect',"hardware::TLvector(Diphoton_pt[0], Diphoton_eta[0], Diphoton_phi[0], Diphoton_mass[0])")
    selection.a.Define('subleadA_vect','hardware::TLvector(Diphoton_pt[1], Diphoton_eta[1], Diphoton_phi[1], Diphoton_mass[1])')
    selection.a.Define('AAmass','hardware::InvariantMass({leadA_vect,subleadA_vect})')
    selection.a.Define('itop','int((Dijet_particleNet_TvsQCD[0]>0.8) || (Dijet_particleNet_TvsQCD[1]<0.8))')
    selection.a.Define('mtp1','hardware::InvariantMass({lead_vect,leadA_vect,subleadA_vect})')
    selection.a.Define('mtp2','hardware::InvariantMass({sublead_vect,leadA_vect,subleadA_vect})')
    selection.a.Define('TPmass','mtp1')
#    selection.a.Define('TPmass','itop*mtp1+(1-itop)*mtp2')
#    selection.a.Define('dipt0','itop*Dijet_pt_corr[0]+(1-itop)*Dijet_pt_corr[1]')
#    selection.a.Define('dipt1','itop*Dijet_pt_corr[1]+(1-itop)*Dijet_pt_corr[0]')
...
#    selection.a.Define('HT','abs(dipt0)+abs(dipt1)')
...
#    selection.a.Define('Aphi1','Aphi1')
	...
	histname = '{}_{}_{}'.format(setname, year, varname)
 	if ('dipt' in varname):
	    hist_tuple = (varname,varname,100,25,2025)
#DP EDIT
...
#            hist_tuple = (varname,varname,100,350,2350)
        if ('diApt' in varname):
            hist_tuple = (varname,varname,100,25,825)
        if ('dimass' in varname):
            hist_tuple = (varname,varname,100,50,2050)
        if ('TPmass' in varname):
            hist_tuple = (varname,varname,100,50,1650)
        if ('AAmass' in varname):
            hist_tuple = (varname,varname,100,25,825)
	if (varname == 'HT'):
	...
	if 'Tag' in varname:
	    hist_tuple = (varname, varname, 64, 0, 3)
        if 'nAA' in varname:
...
#DP EDIT commented out the following line and added the reference to the SWAN fitted efficiency!!!!
...
#DP EDIT make sure this references the correct year!!!
#    args.trigEff = Correction("TriggerEff16",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT0_16.root','Pretag'], corrtype='weight')
#    args.trigEff = Correction("TriggerEff17",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_17XB.root','Pretag'], corrtype='weight')
...
#        args.trigEff = Correction("TriggerEff16APV",'EffLoader_2DfittedHist.cc',['out_Eff_2016APV.root','Eff_2016APV'],corrtype='weight')
       args.trigEff = Correction("TriggerEff16APV",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_16APV.root','Pretag'], corrtype='weight')
    elif ('17' in args.era):
#        args.trigEff = Correction("TriggerEff17",'EffLoader_2DfittedHist.cc',['out_Eff_2017.root','Eff_2017'],corrtype='weight')
       ...
       if rand < cutoff:       # apply the 2017B trigger efficiency to this MC
            ...
            args.trigEff = Correction("TriggerEff17",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_17B.root','Pretag'], corrtype='weight')
       else:
            args.trigEff = Correction("TriggerEff17",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_17XB.root','Pretag'], corrtype='weight')
    elif ('18' in args.era):
#        args.trigEff = Correction("TriggerEff18",'EffLoader_2DfittedHist.cc',['out_Eff_2018.root','Eff_2018'],corrtype='weight')
       args.trigEff = Correction("TriggerEff18",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_18.root','Pretag'], corrtype='weight')
    else:
#        args.trigEff = Correction("TriggerEff16",'EffLoader_2DfittedHist.cc',['out_Eff_2016.root','Eff_2016'],corrtype='weight')
       args.trigEff = Correction("TriggerEff16",'TIMBER/Framework/include/EffLoader.h',['THtrigger2D_HT900_16.root','Pretag'], corrtype='weight')
...
#    for setname in ['Data', 'WJetsHT400', 'WJetsHT600', 'WJetsHT800','ZJetsHT400','ZJetsHT600', 'ZJetsHT800', 'ttbar-allhad', 'ttbar-semilep', 'QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000']:
    for setname in ['Data', 'WJetsHT600', 'WJetsHT800', 'ZJetsHT600', 'ZJetsHT800','ttbar-gg','ttbar-gJets','ttbar-allhad','ttbar-semilep', 'QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000']:
...
#DP EDIT
    ...
    '''
#    for setname in ['Data', 'WJetsHT400','WJetsHT600', 'WJetsHT800', 'ZJetsHT400', 'ZJetsHT600', 'ZJetsHT800', 'ttbar-gg','ttbar-gJets','ttbar-allhad', 'ttbar-semilep', 'QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000']:
    for setname in ['Data', 'WJetsHT600', 'WJetsHT800', 'ZJetsHT600', 'ZJetsHT800','ttbar-gg','ttbar-gJets','ttbar-allhad', 'ttbar-semilep', 'QCDHT700','QCDHT1000','QCDHT1500','QCDHT2000']:
