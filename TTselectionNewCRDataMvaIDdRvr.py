...
#    selection = THClass('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    selection = TTClassdRData('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
...
#    selection.ApplyTrigs(args.trigEff)
    ...
    out = ROOT.TFile.Open('rootfilesdRVR/THselection_mvaID_dR_VR_HT%s_%s%s_%s%s.root'%(args.HT, args.setname,
...
#            selection.cuts[t+'MD_HbbvsQCD'] = float(args.topcut)
	    ...
	    selection.a.SetActiveNode(kinOnly)
            if ('16APV' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.49, photon_tagger, 0.8)
            elif ('17' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.58, photon_tagger, 0.8)
            elif ('18' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.58, photon_tagger, 0.8)
            else:
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.50, photon_tagger, 0.8)
	    # SIGNAL REGION
            print('----------------------- SIGNAL REGION --------------------------------------------------------------')
            selection.a.SetActiveNode(kinOnly)
#DP EDIT
            if ('16APV' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.49, photon_tagger, 0.8)
            elif ('17' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.58, photon_tagger, 0.8)
            elif ('18' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.58, photon_tagger, 0.8)
            else:
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.50, photon_tagger, 0.8)
