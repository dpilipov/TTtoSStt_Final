...
#DP EDIT
from TTClass_JMC import TTClass_JMC
...
#    print("Performing the CORRECT CUT")
#    analyzer.Cut("CORRECT_{}_cut".format(SRorCR),"mth==mthX")
#    nCorrect = analyzer.DataFrame.Sum("genWeight").GetValue()
...
#DP EDIT
    ...
    print("nTot = {}".format(nTot))
    print("Performing the CORRECT CUT")
#    analyzer.Cut("CORRECT_{}_cut".format(SRorCR),"topIDcorrect")
#    analyzer.Cut("CORRECT_{}_cut".format(SRorCR),"!topIDcorrect")
#    analyzer.Cut("CORRECT_{}_cut".format(SRorCR),"topIDcorrect || !topIDcorrect")
#    nCorrect = analyzer.DataFrame.Sum().GetValue()
#    print("nCorrect= {}".format(nCorrect))
    if (SRorCR == 'SR'):
       analyzer.Cut("Eff_{}_cut".format(SRorCR),"(Dijet_{0}[0] > {1}) && (Dijet_{0}[1] > {1})".format(Toptagger, ToptaggerWP))
       nTSR = analyzer.DataFrame.Sum("genWeight").GetValue()
...
#    selection = THClass('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    selection = TTClass_JMC('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    ...
    print("Performing the CORRECT CUT")
#    selection.a.Cut("CORRECT_cut","topIDcorrect")
#    selection.a.Cut("CORRECT_cut","!topIDcorrect")
#    analyzer.Cut("CORRECT_{}_cut".format(SRorCR),"topIDcorrect || !topIDcorrect")
#    nCorrect = analyzer.DataFrame.Sum().GetValue()
#    print("nCorrect= {}".format(nCorrect))
...
#	    TopVar = 2
        if ('up' in args.variation):
            TopVar = 1
        elif ('down' in args.variation):
            TopVar = 2
    ...
    out = ROOT.TFile.Open('rootfiles/THselectionX_mvaID_HT%s_%s%s_%s%s.root'%(args.HT, args.setname,
...
#            eff_CR_PASS, eff_CR_FAIL, eff_CR_FAILFAIL = getSaaEfficiencies(selection.a, 'CR', top_tagger, 0.8, photon_tagger, 1)
            eff_CR_PASS, eff_CR_FAIL, eff_CR_FAILFAIL = getSaaEfficienciesTag(selection.a, 'CR', top_tagger, 0.8, photon_tagger, 1)
	...
	# rkey: SR/CR, pfkey: pass/loose/fail
            ...
            for rkey,rpair in {"SR":passfailSR_SF,"SR1":passfailCR_SF}.items():
              for pfkey,n in rpair.items():
                ...
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_mvaID'),16,25,825,29,125,3025),['Smass','mth'])
                templates.Do('Write')
...
#    else
#DP EDIT - just use 2016 for all the years!
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
