...
def generateCutflow(setName, HT=850, selection=True):
    '''
...
#      if (year==16):
    ...
    if selection:
        ...
        for year, varDict in years.items():
#          if (year==16):
	    if not os.path.exists('rootfiles/THselection_mvaID_HT{}_{}_{}.root'.format(HT, setName, year)):
	        pass
	    f = ROOT.TFile.Open('rootfiles/THselection_mvaID_HT{}_{}_{}.root'.format(HT, setName, year))
            print(f)
	    h = f.Get('cutflow')
...
#    mPhi = 250
    mPhi = 250
#    Tmasses = [900,1000,1100,1200,1400,1500]
...
#      for mPhi in phiMasses:
	nProc = 0
#DP EDIT
	res = generateCutflow('{}{}x{}'.format(prefix, Tmass, mPhi), 850, selection)
