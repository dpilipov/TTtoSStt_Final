...
#DP EDIT FOR HEM: drop Data, and use random variable to scale the MC
...
#                    self.a.Cut('HEM','%s[0] > 0'%(HEM_worker.GetCall(evalArgs={"FatJet_eta":"Dijet_eta","FatJet_phi":"Dijet_phi"})))
                    print('Data, 2018, hence perform HEM cut')
                    self.a.Define('NewWeights','HEM_dropData(Dijet_eta,Dijet_phi,run)')
                    self.a.Cut('HEM_veto_Data','NewWeights[0] > 0')
            else:
...
#DP EDIT changed the manner in which the HEM correction is performed on MC - instead of scaling, we use random reweighing
#                    self.a.AddCorrection(
#                        Correction('HEM_drop','TIMBER/Framework/include/HEM_drop.h',[self.setname],corrtype='corr')
#                    )
                    print('MC, 2018, hence perform the HEM correction...')
                    self.a.Define('NewWeightsMC','HEM_dropMC(Dijet_eta,Dijet_phi)')
                    self.a.Cut('HEM_veto_MC','NewWeightsMC[0]>0.0')
...
#                    self.a.AddCorrection(Correction('HEM_drop',corrtype='corr'))
                    print('MC, 2018, hence perform the HEM correction...')
                    self.a.Define('NewWeightsMC2','HEM_dropMC(Dijet_eta,Dijet_phi)')
                    self.a.Cut('HEM_veto_MC2','NewWeightsMC2[0]>0.0')
                if 'ttbar' in self.setname:
...
#        self.a.Define('mth','topchoice2*mth1+(1-topchoice2)*mth2')
    ...
    def OpenForSelection(self,variation):
        ...
        self.a.AddCorrection(genWCorr, evalArgs={'val':'genWeight'})
        if not self.a.isData:
          print('not Data, add L1PreFire')
          if not (self.year == "18"):
            print('but not for 2018')
            genWCorr    = Correction('genW','TIMBER/Framework/TopPhi_modules/BranchCorrection.cc',corrtype='corr',mainFunc='evalCorrection') # workaround so we can have multiple BCs
            self.a.AddCorrection(genWCorr, evalArgs={'val':'genWeight'})
...
#            self.a.MakeWeightCols(extraNominal='genWeight' if not self.a.isData else '')
        ...
        if ('16APV' in self.year):
              self.a.Define('Top1Cat','int(Dijet_particleNet_TvsQCD[0]>0.49)')
              self.a.Define('Top2Cat','int(Dijet_particleNet_TvsQCD[0]>0.49)')
              self.a.Define('JetSF1','getSFwp10(Top1Cat,Dijet_pt[0],"2016APV","0")')
              self.a.Define('JetSF2','getSFwp10(Top2Cat,Dijet_pt[1],"2016APV","0")')
        elif ('16' in self.year):
              self.a.Define('Top1Cat','int(Dijet_particleNet_TvsQCD[0]>0.50)')
              self.a.Define('Top2Cat','int(Dijet_particleNet_TvsQCD[0]>0.50)')
              self.a.Define('JetSF1','getSFwp10(Top1Cat,Dijet_pt[0],"2016","0")')
              self.a.Define('JetSF2','getSFwp10(Top2Cat,Dijet_pt[1],"2016","0")')
        elif ('17' in self.year):
              self.a.Define('Top1Cat','int(Dijet_particleNet_TvsQCD[0]>0.58)')
              self.a.Define('Top2Cat','int(Dijet_particleNet_TvsQCD[0]>0.58)')
              self.a.Define('JetSF1','getSFwp10(Top1Cat,Dijet_pt[0],"2017","0")')
              self.a.Define('JetSF2','getSFwp10(Top2Cat,Dijet_pt[1],"2017","0")')
        else:
              self.a.Define('Top1Cat','int(Dijet_particleNet_TvsQCD[0]>0.58)')
              self.a.Define('Top2Cat','int(Dijet_particleNet_TvsQCD[0]>0.58)')
              self.a.Define('JetSF1','getSFwp10(Top1Cat,Dijet_pt[0],"2018","0")')
              self.a.Define('JetSF2','getSFwp10(Top2Cat,Dijet_pt[1],"2018","0")')
...
#        self.a.Define('topchoice','int(dR0 > 0.8)*(int(dR1<2.0)+int(dR1>2.0)*int(dR0>dR1))')
        self.a.Define('topchoice','int(dR0 > 0.8)*int(dR0>dR1)')
        self.a.Define('mth','topchoice*mth1+(1-topchoice)*mth2')
