...
#selection.a.Cut('photonMvaID','Diphoton2_mvaID[0] > -1.01 && Diphoton2_mvaID[1] > -1.01')
...
jets.Add('ilead','1-int((Dijet_particleNet_TvsQCD[0] > 0.8 && Dijet_particleNet_TvsQCD[1] > 0.8)||(Dijet_particleNet_TvsQCD[0] < 0.8 && Dijet_particleNet_TvsQCD[1] < 0.8)||(Dijet_particleNet_TvsQCD[0] > 0.8 && Dijet_particleNet_TvsQCD[1] < 0.8))')
jets.Add('gen_vect',"hardware::TLvector(GenPart_pt[topID], GenPart_eta[topID], GenPart_phi[topID], GenPart_mass[topID])")
jets.Add('dR0','hardware::DeltaR(lead_vect,gen_vect)')
jets.Add('dR1','hardware::DeltaR(sublead_vect,gen_vect)')
jets.Add('imatch','int((dR0<0.8)||(dR1<0.8))')
jets.Add('iJetMatch','imatch*int(dR1<dR0)-(1-imatch)')
#jets.Add('iCorrect','int(ilead == iJetMatch)')
...
#plotting_vars.Add('ilead','1-int((Dijet_particleNet_TvsQCD[0] > 0.8 && Dijet_particleNet_TvsQCD[1] > 0.8)||(Dijet_particleNet_TvsQCD[0] < 0.8 && Dijet_particleNet_TvsQCD[1] < 0.8)||(Dijet_particleNet_TvsQCD[0] > 0.8 && Dijet_particleNet_TvsQCD[1] < 0.8))')
#plotting_vars.Add('iJetMatch','imatch*int(dR1<dR0)-(1-imatch)')
plotting_vars.Add('dRgenJet','imatch*((1-iJetMatch)*dR0+iJetMatch*dR1)-(1-imatch)')
#plotting_vars.Add('iCorrect','int(ilead == iJetMatch)')
#plotting_vars.Add('StoAAid0','FindID0StoAA(nGenPart,GenPart_pdgId,GenPart_genPartIdxMother)')
#plotting_vars.Add('topID','FindIDtopWithS(StoAAid0,nGenPart,GenPart_pdgId,GenPart_genPartIdxMother)')
#plotting_vars.Add('SmassCutBased','hardware::InvariantMass({Alead_vect,Asublead_vect})')
#plotting_vars.Add('TPmassCutBased','(1-ilead)*hardware::InvariantMass({lead_vect,Alead_vect,Asublead_vect})+ilead*hardware::InvariantMass({sublead_vect,Alead_vect,Asublead_vect})')
plotting_vars.Add('SmassMvaID','hardware::InvariantMass({Alead2_vect,Asublead2_vect})')
#plotting_vars.Add('TPmassMvaID','(1-ilead)*hardware::InvariantMass({lead_vect,Alead2_vect,Asublead2_vect})+ilead*hardware::InvariantMass({sublead_vect,Alead_vect,Asublead_vect})')
plotting_vars.Add('TPmassMvaID','imatch*((1-iJetMatch)*hardware::InvariantMass({lead_vect,Alead2_vect,Asublead2_vect})+iJetMatch*hardware::InvariantMass({sublead_vect,Alead_vect,Asublead_vect}))')
...
NCuts.Add('AptMvaID_cut','Diphoton2_pt[0] > 0 && Diphoton2_pt[1] > 0')
...
#NCuts.Add('photonCutBased_cut','Diphoton_cutBased[0] > 0 && Diphoton_cutBased[1] > 0')
#NCuts.Add('photonMvaID_cut','(Diphoton_mvaID[0] > -0.91 && Diphoton_mvaID[1] < -0.91) || (Diphoton_mvaID[0] < -0.91 && Diphoton_mvaID[1] > -0.91)')
NCuts.Add('photonMvaID_cut','Diphoton2_mvaID[0] > 0.8 && Diphoton2_mvaID[1] > 0.8')
...
#NCuts.Add('StoAAid0_cut','StoAAid0> -2')
#NCuts.Add('topID_cut','topID> -2')
#NCuts.Add('ilead_cut','ilead> -1')
#NCuts.Add('iJetMatch_cut','iJetMatch > -1')
NCuts.Add('dRgenJet_cut','dRgenJet < 0.8')
#NCuts.Add('iCorrect_cut','iCorrect>0')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_v10cX2_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
#    'mtop':[25,0,300],
#    'mtphi':[25,800,2200],
...
for nkey in nminus1Nodes.keys():
    ...
    print('\t{}'.format(nkey))
#    if ('StoAAid0' not in nkey) and ('StoAAid1' not in nkey) and ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('iJetMatch' not in nkey) and ('dRgenJet' not in nkey) and ('TPmassCutBased' not in nkey) and ('dPhi' not in nkey) and ('AdPhi' not in nkey) and ('photonMvaID' not in nkey) and ('tmass' not in nkey) and ('AptCutBased' not in nkey) and ('AptMvaID' not in nkey) and ('TPmassMvaID' not in nkey) and ('SmassMvaID' not in nkey) and ('SmassCutBased' not in nkey) and ('deltaR' not in nkey) and ('photonCutBased' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
