...
#DP EDIT
from TTClass17 import TTClass17
...
#    selection = THClass('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
    selection = TTClass17('dijet_nano/{}_{}_snapshot.txt'.format(args.setname,args.era),args.era,1,1)
...
#            eA1CR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='cr2')
            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2017",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_CR_PASS, eff_CR_FAIL))
...
#            eA1SR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='sr2')
            selection.a.Define('DiPhotonCatSF','updatePhotonTag(DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2017",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_SR_PASS, eff_SR_FAIL))
...
#    args.trigEff = Correction("TriggerEff%s"%args.era,'EffLoader_2DfittedHist.cc',['out_Eff_20%s.root'%args.era,'Eff_20%s'%args.era],corrtype='weight')
    args.trigEff = Correction("TriggerEff17",'EffLoader_2DfittedHist.cc',['out_Eff_2016APV.root','Eff_2016APV'],corrtype='weight')
#    else:
