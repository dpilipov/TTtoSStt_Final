...
plotting_vars.Add('photonNOTelecTag','Diphoton_electronVeto[0]')

# N-1 cuts to make
NCuts = CutGroup('NCuts')
NCuts.Add('deltaEta_cut','deltaEta < 1.6')
NCuts.Add('pt_cut','Dijet_pt[0] > 400 && Dijet_pt[1] > 400')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_v1_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
    'mtop':[25,0,300],
#    'mtphi':[25,800,2200],
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('photonNOTelecTag' not in nkey) and ('Apt' not in nkey) and ('Smass' not in nkey) and ('deltaR' not in nkey) and ('photonTag' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
