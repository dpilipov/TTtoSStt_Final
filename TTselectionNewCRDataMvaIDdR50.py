...
#    selection.ApplyTrigs(args.trigEff)
    ...
    out = ROOT.TFile.Open('rootfilesdR/THselection_mvaID_dR50_HT%s_%s%s_%s%s.root'%(args.HT, args.setname,
...
#            selection.cuts[t+'MD_HbbvsQCD'] = float(args.topcut)
	    ...
	    selection.a.SetActiveNode(kinOnly)
            if ('16APV' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.74, photon_tagger, 0.5)
            elif ('17' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.8, photon_tagger, 0.5)
            elif ('18' in args.era):
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.8, photon_tagger, 0.5)
            else:
               passfailCR = selection.ApplySTagTopTagTag('CR', top_tagger, 0.73, photon_tagger, 0.5)
	    # SIGNAL REGION
            print('----------------------- SIGNAL REGION --------------------------------------------------------------')
            selection.a.SetActiveNode(kinOnly)
#DP EDIT
            if ('16APV' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.74, photon_tagger, 0.5)
            elif ('17' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.8, photon_tagger, 0.5)
            elif ('18' in args.era):
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.8, photon_tagger, 0.5)
            else:
               passfailSR = selection.ApplySTagTopTagTag('SR', top_tagger, 0.73, photon_tagger, 0.5)
