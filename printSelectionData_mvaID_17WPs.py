import ROOT
f = ROOT.TFile.Open('rootfilesdR/THselection_mvaID_dR_HT900_Data_17.root','read')
f1 = ROOT.TFile.Open('rootfilesdR/THselection_mvaID_dR50_HT900_Data_17.root','read')
f2 = ROOT.TFile.Open('rootfilesdR/THselection_mvaID_dR40_HT900_Data_17.root','read')
f3 = ROOT.TFile.Open('rootfilesdR/THselection_mvaID_dRWP80_HT900_Data_17.root','read')
# get the histos
h1 = f.Get("MtpvMs_TvsQCD_mvaID_SR_pass__nominal")
h11 = f1.Get("MtpvMs_TvsQCD_mvaID_SR_pass__nominal")
h12 = f2.Get("MtpvMs_TvsQCD_mvaID_SR_pass__nominal")
h13 = f3.Get("MtpvMs_TvsQCD_mvaID_SR_pass__nominal")
#h2 = f.Get("MtpvMs_TvsQCD_mvaID_SR_fail__nominal")
#h3 = f.Get("MtpvMs_TvsQCD_mvaID_CR_pass__nominal")
h4 = f.Get("MtpvMs_TvsQCD_mvaID_CR_fail__nominal")
h41 = f1.Get("MtpvMs_TvsQCD_mvaID_CR_fail__nominal")
h42 = f2.Get("MtpvMs_TvsQCD_mvaID_CR_fail__nominal")
h43 = f3.Get("MtpvMs_TvsQCD_mvaID_CR_fail__nominal")
h1.SetTitle("Data: SR, mvaID>0.8")
h11.SetTitle("Data: SR, mvaID>0.5")
h12.SetTitle("Data: SR, mvaID>0.4")
h13.SetTitle("Data: SR, mvaID_WP80")
#h3.SetTitle("Data: 2t1a")
h4.SetTitle("Data: CR, mvaID>0.8")
h41.SetTitle("Data: CR, mvaID>0.5")
h42.SetTitle("Data: CR, mvaID>0.4")
h43.SetTitle("Data: CR, mvaID_WP80") 
#h11 = f.Get("MtpvMs_TvsQCD_mvaID_SR1_pass__nominal")
#h13 = f.Get("MtpvMs_TvsQCD_mvaID_CR1_pass__nominal")
#h14 = f.Get("MtpvMs_TvsQCD_mvaID_CR1_fail__nominal")
#h11.SetTitle("ttbar MC: 1t2a")
#h13.SetTitle("ttbar MC: 1t1a")
#h14.SetTitle("ttbar MC: 1t0a")

#h5 = f.Get("MtpvMs_TvsQCD_mvaID_SR_failfail__nominal")
#h6 = f.Get("MtpvMs_TvsQCD_mvaID_CR_failfail__nominal")

# make a canvas
c = ROOT.TCanvas('c','c')
c.Divide(4,2)
#c.cd()
#c.Print('outputSelection.pdf[')  # this tells the canvas to open file for multi-page printing
#c.Clear()

# loop over the histos u want to draw
i=0
for h in [h1, h11, h12, h13, h4, h41, h42, h43]:
#    c.Clear()   # clear any previous draws from the canvas
    i = i+1
    c.cd(i)
    h.SetStats(0)
    zl=h.GetMaximum()
    print(zl)
    if (zl<1.0):
       h.SetMaximum(1.0)
       print('1.0')
    if (i>0):
       h.Draw("colz")  # draw the histo with colz
#    c.Print("outputSelection.pdf")  # save it to the pdf
#c.Clear()   # clear any previous draws from the canvas
#h5.SetStats(0)
#h5.SetMaximum(3.00)
#h5.Draw("colz")  # draw the histo with colz
c.Print("outputSelectionData_mvaIDwp_17.pdf")  # save it to the pdf
#c.Print("outputSelection.pdf]")  # finished, close the multipage file
