...
jets.Add('diff1','abs(dRj1-2.6)')
jets.Add('ichoice','int(diff0<diff1)')
plotting_vars.Add('dRselection','ichoice*dRj0+(1-ichoice)*dRj1')
...
jets.Add('mth1','hardware::InvariantMass({Top1_vect,Photon1_vect,Photon2_vect})')
jets.Add('mth2','hardware::InvariantMass({Top2_vect,Photon1_vect,Photon2_vect})')
plotting_vars.Add('mth','ichoice*mth1+(1-ichoice)*mth2')
...
NCuts.Add('dRselection','dRselection > 0.0')
NCuts.Add('dRsignal','dRsignal > 0.0')
NCuts.Add('dRreflection','dRreflection > 0.0')
NCuts.Add('dtopR','dtopR < 0.8')
NCuts.Add('mth','mth > 0.0 && mth < 3000.0')
...
oFile = ROOT.TFile.Open('rootfiles/NMinus1_dRsel0p8_{}_{}_{}of{}.root'.format(args.setname, args.era, args.ijob, args.njobs),'RECREATE')
oFile.cd()
binning = {
#    'mth1':[25,200,3025],
#    'mth2':[25,200,3025],
...
#    if ('mtop' not in nkey) and ('pt0' not in nkey) and ('pt1' not in nkey) and ('deltaEta' not in nkey) and ('topTag0' not in nkey) and ('topTag1' not in nkey):
    if ('mth'  not in nkey) and ('dRselection'  not in nkey) and ('dRgen'  not in nkey) and ('dR0'  not in nkey) and ('dR1'  not in nkey) and ('dRsignal' not in nkey) and ('dRreflection' not in nkey) and ('dtopR' not in nkey) and ('photonMvaID' not in nkey) and ('tmass' not in nkey) and ('AptCutBased' not in nkey) and ('AptMvaID' not in nkey) and ('TPmassMvaID' not in nkey) and ('SmassMvaID' not in nkey) and ('SmassCutBased' not in nkey) and ('deltaR' not in nkey) and ('photonCutBased' not in nkey) and ('mtop' not in nkey) and ('pt' not in nkey) and ('deltaEta' not in nkey) and ('topTag' not in nkey):
	continue
    print('2')	
