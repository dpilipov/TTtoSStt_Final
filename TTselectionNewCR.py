...
#	    TopVar = 2
    ...
    out = ROOT.TFile.Open('rootfiles/THselection_cutBased_HT%s_%s%s_%s%s.root'%(args.HT, args.setname,
...
#            eA1CR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='cr2')
            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(0,DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2018",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_CR_PASS, eff_CR_FAIL))
#            selection.a.Define('DiPhotonCatCRSF','updatePhotonTag(0,DiPhotonCat,DiphotoniTag_pt,DiphotonTag_eta,DiphotonTag_cutBased,1.0,{0},{1})'.format(eff_CR_PASS, eff_CR_FAIL))
...
#            eff_SR_PASS, eff_SR_FAIL, eff_SR_FAILFAIL = getSaaEfficiencies(selection.a, 'SR', top_tagger, 0.8, photon_tagger, 1)
            eff_SR_PASS, eff_SR_FAIL, eff_SR_FAILFAIL = getSaaEfficienciesTag(selection.a, 'SR', top_tagger, 0.8, photon_tagger, 1)
...
#            eA1SR = getPhotonEfficiencies(analyzer=selection.a, tagger='Diphoton_'+photon_tagger+'[1]', wp=1, idx=1, tag='sr2')
            selection.a.Define('DiPhotonCatSF','updatePhotonTag(0,DiPhotonCat,Diphoton_pt,Diphoton_eta,Diphoton_cutBased,1.0,{0},{1},"2018",Diphoton_r9,Diphoton_isScEtaEE,Diphoton_isScEtaEB)'.format(eff_SR_PASS, eff_SR_FAIL))
#            selection.a.Define('DiPhotonCatSF','updatePhotonTag(0,DiPhotonCat,DiphotonTag_pt,DiphotonTag_eta,DiphotonTag_cutBased,1.0,{0},{1})'.format(eff_SR_PASS, eff_SR_FAIL))
	...
	# rkey: SR/CR, pfkey: pass/loose/fail
            ...
            for rkey,rpair in {"SR":passfailSR_SF,"SR1":passfailCR_SF}.items():
              for pfkey,n in rpair.items():
                ...
                if (rkey=='SR'):
                  if (pfkey=='fail'):
                    mod_name = "%s_%s_%s"%('TvsQCD_cutBased',"CR","pass")
                    mod_title = "%s %s"%("CR","pass")
                  elif (pfkey=='failfail'):
                    mod_name = "%s_%s_%s"%('TvsQCD_cutBased',"CR","fail")
                  ...
                  if (pfkey=='fail'):
                    mod_name = "%s_%s_%s"%('TvsQCD_cutBased',"CR1","pass")
                    mod_title = "%s %s"%("CR1","pass")
                  elif (pfkey=='failfail'):
                    mod_name = "%s_%s_%s"%('TvsQCD_cutBased',"CR1","fail")
                    mod_title = "%s %s"%("CR1","fail")
                  else:
                    mod_name = "%s_%s_%s"%('TvsQCD_cutBased',"SR1","pass")
                    mod_title = "%s %s"%("SR1","pass")
                print(mod_name)
                print(mod_title)
                selection.a.SetActiveNode(n)
                templates = selection.a.MakeTemplateHistos(ROOT.TH2F('MtpvMs_%s'%mod_name,'MtpvMs %s with %s'%(mod_title,'TvsQCD_cutBased'),16,25,825,26,425,3025),['Smass','mth'])
                templates.Do('Write')
...
if __name__ == '__main__':
    ...
    if (args.era == '17'):
        cutoff = 0.059053        # fraction of total JetHT data belonging to 2017B (743/12582)
        TRand = ROOT.TRandom()
