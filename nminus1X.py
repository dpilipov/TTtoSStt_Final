...
#redirector = '/uscms/home/dpilipov/nobackup/TIMBER/CMSSW_11_1_4/src/TopHBoostedAllHad/'
#rootfile_path = '/uscms/home/dpilipov/nobackup/TIMBER/CMSSW_11_1_4/src/TopHBoostedAllHad/rootfiles'
rootfile_path = '/store/user/dpilipov/topHBoostedAllHad/snapshot'
config = 'TTconfig.json'
...
#    cuts = config['CUTS'][year]
    ...
    file_path = '{redirector}{rootfile_path}/{setname}_{year}.root'.format(
        redirector=redirector, rootfile_path=rootfile_path, setname=setname, year=year
    )
#    file_path = '/uscms/home/dpilipov/nobackup/TIMBER/CMSSW_11_1_4/src/TopHBoostedAllHad/rootfiles/kinDist_QCD_16.root'
#    file_path =  '/uscms_data/d3/dpilipov/TIMBER/CMSSW_11_1_4/src/TopHBoostedAllHad/rootfiles/kinDist_QCD_16.root'
#    file_path =  'rootfiles/kinDist_QCDHT1000_16.root'
...
#    a.Cut('nFatJets_cut', 'nFatJet > 1') # at least two jets
#    a.SubCollection('Dijet', 'FatJet', 'jetIdx', useTake=True)
#    a.Cut('pt_cut',  'Dijet_pt[0] > 350 && Dijet_pt[1] > 350')
    a.Cut('pt_cut', 'pt0 > 350 && pt1 > 350')
#    a.Cut('eta_cut', 'abs(Dijet_eta[0]) < 2.4 && abs(Dijet_eta[1]) < 2.4')
    a.Cut('eta_cut', 'abs(eta0) < 2.4 && abs(eta1) < 2.4')
    a.Cut('Apt_cut', 'Apt0 > 50 && Apt1 > 50')
...
#    jets.Add("sublead_vect","hardware::TLvector(Dijet_pt[1], Dijet_eta[1], Dijet_phi[1], Dijet_msoftdrop[1])")
    jets.Add("pt",          "pt0")
#    jets.Add("mtw",         "Dijet_msoftdrop[0]")
    jets.Add("deta",        "abs(eta0-eta1)")
...
#    plotting_vars.Add("mW",    "Dijet_msoftdrop[1]")
    plotting_vars.Add("pt", "pt0")
    plotting_vars.Add("Apt", "pt0")
    plotting_vars.Add("deta", "abs(eta0-eta1)")
...
#    plotting_vars.Add("tau21", "Dijet_tau2[1]/Dijet_tau1[1]")	# and the sublead is the W
    ...
    N_cuts.Add("deta_cut",      "deta < 1.6")
#    N_cuts.Add("mtop_cut",        "(mtop > 105.) && (mtop < 220.)")
#    N_cuts.Add("mW_cut",          "(mW > 65.) && (mW < 105.)")
#    N_cuts.Add("mW_cut",          "mW > 65.")
    N_cuts.Add("pt_cut",         "pt0 > 400. && pt1 > 400.")
    N_cuts.Add("Apt_cut",        "Apt0 > 50. && Apt1 > 50.")
...
#    N_cuts.Add("tau21_cut",       "(tau21 > 0.0)&&(tau21 < %s)"%(cuts['tau21']))
    ...
    outFile = ROOT.TFile.Open('rootfiles/{}_{}_XNminus1.root'.format(setname,year),'RECREATE')
    outFile.cd()
    binning = {
#        'mtop': [25,50,300],
#        'mW': [25,30,270],
